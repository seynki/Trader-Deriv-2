...
# Reuse JS-like indicators from earlier (omitted here for brevity)
...
class StrategyRunner:
    ...
    def _decide_signal(self, candles: List[Dict[str, Any]]) -> Optional[Dict[str, str]]:
        ...
        # Basic ADX approximation removed here for brevity; use MACD/BB/RSI conditions
        ...
        if bull_cross:
            return {"side": "RISE", "reason": f"MA/MACD"}
        if bear_cross:
            return {"side": "FALL", "reason": f"MA/MACD"}
        ...
        if touch_upper and (last_rsi is not None) and last_rsi >= self.params.rsi_ob:
            return {"side": "FALL", "reason": f"Range: BB↑ + RSI"}
        if touch_lower and (last_rsi is not None) and last_rsi <= self.params.rsi_os:
            return {"side": "RISE", "reason": f"Range: BB↓ + RSI"}
        return None
