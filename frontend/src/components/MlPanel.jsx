...
export default function MlPanel() {
  ...
  const [source, setSource] = useState("file"); // Changed default to 'file' instead of 'mongo'
                        ...
                        <div className="max-h-72 overflow-auto">
                          {lastResult.grid.map((row, idx) => (
                            <div key={idx} className="grid grid-cols-6 text-xs border-t border-emerald-500/10">
                              ...
                              <div className="px-2 py-1">{row.metrics?.precision != null ? Number(row.metrics.precision).toFixed(3) : "-"}</div>
                              <div className="px-2 py-1">{row.backtest?.ev_per_trade != null ? Number(row.backtest.ev_per_trade).toFixed(3) : "-"}</div>
                              <div className="px-2 py-1">{row.metrics?.trades_per_day != null ? Number(row.metrics.trades_per_day).toFixed(2) : "-"}</div>
